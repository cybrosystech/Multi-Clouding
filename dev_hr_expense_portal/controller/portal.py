...
##    
    @http.route(['/my/hr_expense/<int:order_id>'], type='http', auth="public", website=True)
    def portal_hr_expense_page(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            hr_expense_sudo = self._document_check_access('hr.expense', order_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        now = fields.Date.today()
#        if report_type in ('html', 'pdf', 'text'):
#        	return self._show_report(model=stock_scrap_sudo, report_type=report_type, report_ref='dev_scrap_report.menu_multi_scrap_print', download=download)
        if hr_expense_sudo and request.session.get('view_scrap_%s' % hr_expense_sudo.id) != now and request.env.user.share and access_token:
            request.session['view_rma_%s' % hr_expense_sudo.id] = now
            body = _('Leave viewed by customer')
            _message_post_helper(res_model='hr.expense', res_id=hr_expense_sudo.id, message=body, token=hr_expense_sudo.access_token, message_type='notification', subtype="mail.mt_note", partner_ids=hr_expense_sudo.user_id.sudo().partner_id.ids)
        values = {
        ...
        return request.render('dev_hr_expense_portal.expense_portal_template', values)
    ...
    @http.route('/create/hr_expense', auth='public', website=True, type='http', csrf_token=True)
    def create_hr_expense(self, **kw):
    	values = self.get_default_data_leave()
    	return request.render('dev_hr_expense_portal.website_expense_request', values)
