...
from odoo.tools import SQL, profile,float_round

_logger = logging.getLogger(__name__)


class TascBankReconciliationReportCustomHandler(models.AbstractModel):
    _name = 'tasc.account.bank.reconciliation.report.handler'
    _inherit = 'account.report.custom.handler'
    _description = 'TASC Bank Reconciliation Report Custom Handler'
    ...
    def _custom_options_initializer(self, report, options,
        ...
        if 'active_id' in self._context and self._context.get(
            ...
            options['bank_reconciliation_report_journal_id'] = self.env[
                'account.journal'].search([('type', '=', 'bank')], limit=1).id

        # Remove multi-currency columns if needed
        is_multi_currency = report.user_has_groups(
            'base.group_multi_currency') and report.user_has_groups(
            'base.group_no_one')
    ...
    # Return function
    ######################
    def _build_custom_engine_result(self, date=None, label=None,
    ...
    # Engine
    ######################
    def _report_custom_engine_forced_currency_amount(self, expressions, options,
                                                     date_scope,
                                                     current_groupby,
                                                     next_groupby, offset=0,
                                                     limit=None, warnings=None):
        _journal, journal_currency, _company_currency = self._get_bank_journal_and_currencies(
            options)
        return self._build_custom_engine_result(
            amount_currency_id=journal_currency.id)

    def _report_custom_engine_unreconciled_last_statement_receipts(self,
                                                                   expressions,
                                                                   options,
                                                                   date_scope,
                                                                   current_groupby,
                                                                   ...
                                                                   limit=None,
                                                                   warnings=None):
        return self._bank_reconciliation_report_custom_engine_common(options,
                                                                     'receipts',
                                                                     current_groupby,
                                                                     True)

    def _report_custom_engine_unreconciled_statement_receipts(self,
                                                                   expressions,
                                                                   options,
                                                                   date_scope,
                                                                   current_groupby,
    ...
    def _report_custom_engine_unreconciled_last_statement_payments(self,
                                                                   expressions,
                                                                   options,
                                                                   date_scope,
                                                                   current_groupby,
    ...
    def _report_custom_engine_unreconciled_receipts(self, expressions, options,
                                                    date_scope, current_groupby,
                                                    next_groupby, offset=0,
                                                    limit=None, warnings=None):
        return self._bank_reconciliation_report_custom_engine_common(options,
                                                                     'receipts',
    ...
    def _report_custom_engine_unreconciled_payments(self, expressions, options,
                                                    date_scope, current_groupby,
                                                    next_groupby, offset=0,
                                                    limit=None, warnings=None):
        return self._bank_reconciliation_report_custom_engine_common(options,
    ...
    def _report_custom_engine_outstanding_receipts(self, expressions, options,
                                                   date_scope, current_groupby,
                                                   next_groupby, offset=0,
                                                   limit=None, warnings=None):
    ...
    def _report_custom_engine_outstanding_payments(self, expressions, options,
                                                   date_scope, current_groupby,
                                                   next_groupby, offset=0,
                                                   limit=None, warnings=None):
        return self._bank_reconciliation_report_custom_engine_outstanding_common(
    ...
    def _report_custom_engine_misc_operations(self, expressions, options,
                                              date_scope, current_groupby,
                                              next_groupby, offset=0,
                                              limit=None, warnings=None):
        ...
        journal, journal_currency, _company_currency = self._get_bank_journal_and_currencies(
            options)
        ...
        misc_operations_amount = self.env["account.move.line"]._read_group(
            ...
            0]  # Needed to get the balance from the tuples given by the read group
        return self._build_custom_engine_result(
            amount=misc_operations_amount or 0,
            amount_currency_id=journal_currency.id)

    def _report_custom_engine_last_statement_balance_amount(self, expressions,
                                                            options, date_scope,
                                                            current_groupby,
                                                            next_groupby,
                                                            offset=0,
        ...
        if current_groupby:
            raise UserError(
                _("Custom engine _report_custom_engine_last_statement_balance_amount does not support groupby"))
    ...
    def _report_custom_engine_statement_balance_amount(self, expressions,
                                                            options, date_scope,
                                                            current_groupby,
                                                            next_groupby,
                                                            offset=0,
        ...
        statement_domain = [('journal_id', '=', journal.id),
                            ('statement_id', '!=', False),
                            ('date', '<=', report_date),
                            ('is_reconciled', '=', False),
                            ('to_check', '=', False)]
        st_line = self.env['account.bank.statement.line'].search(
            statement_domain)
        if st_line:
            amount = sum(st_line.mapped('amount'))
        else:
            amount = 0
        return self._build_custom_engine_result(
            amount=amount,
    ...
    def _report_custom_engine_bank_account_balance_amount(self, expressions,
                                                            options, date_scope,
                                                            current_groupby,
                                                            next_groupby,
                                                            offset=0,
        ...
        journal, journal_currency, _company_currency = self._get_bank_journal_and_currencies(
            options)
        report_date = fields.Date.from_string(options['date']['date_to'])
        domain = [('parent_state','=','posted'),('date', '<=', report_date),
                  ('account_id.id','=',journal.default_account_id.id),
                  ('company_id','=',journal.company_id.id)]
        st_line = self.env['account.move.line'].search(
            domain)
        if st_line:
            amount = sum(st_line.mapped('balance'))

            bank_currency = journal.currency_id or journal.company_id.currency_id
            company_currency = journal.company_id.currency_id

            # Convert the amount to the bank journal currency if needed
            ...
            # Optionally, round the result to the bank journal currency's precision
            amount = float_round(amount,
                                 precision_digits=bank_currency.decimal_places)
    ...
    def _report_custom_engine_bank_suspense_balance_amount(self, expressions,
                                                            options, date_scope,
                                                            current_groupby,
                                                            next_groupby,
                                                            offset=0,
        ...
        journal, journal_currency, _company_currency = self._get_bank_journal_and_currencies(
            options)
        report_date = fields.Date.from_string(options['date']['date_to'])
        domain = [('move_id.journal_id', '=', journal.id),
                  ('date', '<=', report_date),
                  ('account_id.id','=',journal.suspense_account_id.id),
                  ('company_id','=',journal.company_id.id),
                  ('parent_state','=','posted')]
        st_line = self.env['account.move.line'].search(
            domain)
    ...
    def _report_custom_engine_transaction_without_statement_amount(self,
                                                                   expressions,
                                                                   options,
                                                                   date_scope,
                                                                   current_groupby,
                                                                   ...
                                                                   limit=None,
                                                                   warnings=None):
        return self.with_context(unreconciled_without_statement=True)._bank_reconciliation_report_custom_engine_common(options,
                                                                     'all',
                                                                     current_groupby,
                                                                     False,
                                                                     unreconciled=True)

    def _bank_reconciliation_report_custom_engine_common(self, options,
                                                         internal_type,
                                                         current_groupby,
                                                         from_last_statement,
                                                         unreconciled=True):
        ...
        def build_result_dict(query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                if foreign_currency:
                    rate = (res['amount'] / res['amount_currency']) if res[
                        'amount_currency'] else 0

                return self._build_custom_engine_result(
                    date=res['date'] if res['date'] else None,
                    label=res['payment_ref'] or res['ref'] or '/',
                    amount_currency=-res['amount_residual'] if res[
                        'foreign_currency_id'] else None,
                    amount_currency_currency_id=foreign_currency.id if res[
                        'foreign_currency_id'] else None,
                    currency=foreign_currency.display_name if res[
                        'foreign_currency_id'] else None,
                    amount=-res['amount_residual'] * rate if res[
                        'amount_residual'] else None,
                    amount_currency_id=journal_currency.id,
                )
            else:
                amount = 0
                for res in query_res_lines:
                    ...
                    if res['foreign_currency_id']:
                        rate = (res['amount'] / res['amount_currency']) if res[
                            'amount_currency'] else 0
                    amount += -res.get('amount_residual',
                                       0) * rate if unreconciled else res.get(
                        'amount', 0)
        ...
        tables, where_clause, where_params = report._query_get(options,
                                                               'strict_range',
                                                               domain=[
                                                                   (
                                                                   'journal_id',
        ...
        if self.env.context.get('unreconciled_without_statement'):
            last_statement_id_condition = SQL("st_line.statement_id IS NULL")
        else:
            if from_last_statement:
                last_statement_id = self._get_last_bank_statement(journal,
                                                                  options).id
                if last_statement_id:
                    last_statement_id_condition = SQL("st_line.statement_id = %s",
                                                      last_statement_id)
                else:
                    # If there is no last statement, the last statement section must be empty and the other must have all
                    # transaction
                    return self._compute_result([], current_groupby,
                                                build_result_dict)
            else:
                last_statement_id_condition = SQL("st_line.statement_id IS NULL")
        ...
        return self._compute_result(query_res_lines, current_groupby,
                                    build_result_dict)

    def _bank_reconciliation_report_custom_engine_outstanding_common(self,
                                                                     options,
                                                                     internal_type,
                                                                     current_groupby):
        """
            ...
            statement yet
        """
        journal, journal_currency, company_currency = self._get_bank_journal_and_currencies(
            options)
        if not journal:
            return self._build_custom_engine_result()
        ...
        def build_result_dict(query_res_lines):
            if current_groupby == 'id':
                res = query_res_lines[0]
                convert = not (journal_currency and res[
                    'currency_id'] == journal_currency.id)
                amount_currency = res['amount_residual_currency'] if res[
                    'is_account_reconcile'] else res['amount_currency']
                balance = res['amount_residual'] if res[
                    'is_account_reconcile'] else res['balance']
                foreign_currency = self.env['res.currency'].browse(
                    res['currency_id'])
                ...
                return self._build_custom_engine_result(
                    ...
                    amount=company_currency._convert(balance, journal_currency,
                                                     journal.company_id,
                                                     options['date'][
                                                         'date_to']) if convert else amount_currency,
                    amount_currency_id=journal_currency.id,
                )
            else:
                amount = 0
                for res in query_res_lines:
                    ...
                    if convert:
                        balance = res['amount_residual'] if res[
                            'is_account_reconcile'] else res['balance']
                        amount += company_currency._convert(balance,
                                                            journal_currency,
                                                            journal.company_id,
                                                            options['date'][
                                                                'date_to'])
                    else:
                        amount += res['amount_residual_currency'] if res[
                            'is_account_reconcile'] else res['amount_currency']
        ...
        query = SQL(
           ...
           SELECT %(select_from_groupby)s,
             ...
             FROM %(tables)s
             INNER JOIN account_payment ON account_payment.id=account_move_line.payment_id
         ...
         GROUP BY %(group_by)s,
           ...
           """,
            ...
            where_clause=SQL(where_clause, *where_params),
            is_receipt=SQL(
                "account_move_line.balance > 0 and account_payment.payment_type='inbound'") if internal_type == "receipts" else SQL(
                "account_move_line.balance < 0 and account_payment.payment_type='outbound'"),
            group_by=SQL.identifier('account_move_line',
                                    current_groupby) if current_groupby else SQL(
                'account_move_line.account_id'),
            # Same key in the groupby because we can't put a null key in a group by
    ...
    def _compute_result(self, query_res_lines, current_groupby,
                        build_result_dict):
        if not current_groupby:
            ...
            for query_res in query_res_lines:
                ...
                all_res_per_grouping_key.setdefault(grouping_key, []).append(
                    query_res)
    ...
    def _custom_line_postprocessor(self, report, options, lines, warnings=None):
        ...
        inconsistent_statement = self._get_inconsistent_statements(options,
                                                                   journal).ids
        bank_miscellaneous_domain = self._get_bank_miscellaneous_move_lines_domain(
            options, journal)
        ...
        last_statement, balance_gl, balance_end, unexplained_difference, general_ledger_not_matching = self._compute_journal_balances(
            report, options, journal, journal_currency)

        for line in lines:
            line_id = report._get_res_id_from_line_id(line['id'],
                                                      'account.report.line')
            code = self.env['account.report.line'].browse(line_id).code

            if code == "balance_bank":
                line['name'] = _("Balance of '%s'",
                                 journal.default_account_id.display_name)

            if code == "tasc_last_statement_balance":
        ...
        if warnings is not None:
            ...
            if inconsistent_statement:
                warnings['account_reports.inconsistent_statement_warning'] = {
                    'alert_type': 'warning', 'args': inconsistent_statement}
            if has_bank_miscellaneous_move_lines:
                warnings[
                    'account_reports.has_bank_miscellaneous_move_lines'] = {
                    'alert_type': 'warning',
                    'args': journal.default_account_id.display_name}

        return lines

    def _compute_journal_balances(self, report, options, journal,
                                  journal_currency):
        """
        ...
        last_statement, balance_end, difference, general_ledger_not_matching = self._compute_balances(
            options, journal, balance_gl, journal_currency)

        # Format values
        balance_gl = report.format_value(options, balance_gl,
                                         currency=journal_currency,
                                         figure_type='monetary')
        balance_end = report.format_value(options, balance_end,
                                          currency=journal_currency,
                                          figure_type='monetary')
        difference = report.format_value(options, difference,
                                         currency=journal_currency,
                                         figure_type='monetary')
    ...
    def _compute_balances(self, options, journal, balance_gl, report_currency):
        ...
        if last_statement:
            ...
            general_ledger_not_matching = not report_currency.is_zero(
                difference)
    ...
    def _get_last_bank_statement(self, journal, options):
        ...
        """
        report_date = fields.Date.from_string(options['date']['date_to'])
        last_statement_domain = [('journal_id', '=', journal.id),
                                 ('statement_id', '!=', False),
                                 ('date', '<=', report_date)]
    ...
    def _get_bank_miscellaneous_move_lines_domain(self, options, journal):
        ...
        if journal.company_id.fiscalyear_lock_date:
            domain.append(
                ('date', '>', journal.company_id.fiscalyear_lock_date))

        if journal.company_id.account_opening_move_id:
            domain.append(('move_id', '!=',
                           journal.company_id.account_opening_move_id.id))
    ...
    def open_bank_miscellaneous_move_lines(self, options):
        ...
        return {
            ...
            'domain': self.env[
                'account.bank.reconciliation.report.handler']._get_bank_miscellaneous_move_lines_domain(
                options, journal),
        }

    def bank_reconciliation_report_open_inconsistent_statements(self, options,
                                                                params=None):
        """
