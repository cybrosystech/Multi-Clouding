...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        employee = Employee.search([('user_id', '=', request.env.user.id)],
                                   limit=1)
        if employee:
            attendances = attendance_obj.search([('employee_id', '=',
    ...
    @http.route(['/my/attandance', '/my/attandance/page/<int:page>'],
                type='http', auth="public", website=True)
    def portal_attendance_page(self, page=1, date_begin=None, date_end=None,
                               sortby=None, **kw):
        ...
        attendances = attendance_obj.search(domain, limit=self._items_per_page,
                                            offset=pager['offset'])
