...
class GeneralLedgerCustomHandler(models.AbstractModel):
    ...
    def _get_query_amls(self, report, options, expanded_account_ids, offset=0, limit=None):
        ...
        cost_center_name = f"COALESCE(cost_center.name->>'{lang}', cost_center.name->>'en_US')" if \
            self.pool[
                'account.analytic.account'].name.translate else 'cost_center.name'
        for column_group_key, group_options in report._split_options_per_column_group(options).items():
            ...
            query = f'''
                ...
                LEFT JOIN account_analytic_account cost_center on cost_center.id = account_move_line.analytic_account_id
