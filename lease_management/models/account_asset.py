...
class AccountAsset(models.Model):
    ...
    def create_last_termination_move(self, disposal_date):
        ...
        ratio = ((disposal_date - start_month).days + 1) / (
                (end_month - start_month).days + 1)
        if not end_move or not end_move.line_ids:
            raise ValidationError(_('Please post the asset first'))
        new_value = abs(
            end_move.line_ids[0].debit - end_move.line_ids[0].credit) * ratio
        end_move.write({
            'date': disposal_date,
            'line_ids': [(1, line.id, {'debit': new_value if line.debit else 0,
        ...
        if self._context.get('decrease'):
            if move_vals:
                ...
                if int(self.method_period) % 12 != 0:
                    month_days = \
                        calendar.monthrange(first_date.year, first_date.month)[
                            1]
                ...
                for vals in move_vals[:-1]:
                    amount = abs(vals['line_ids'][0][2]['debit'] -
                                 vals['line_ids'][0][2]['credit'])
