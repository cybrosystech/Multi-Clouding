...
class LeaseeContract(models.Model):
        ...
        for rec in self:
            ...
            if rec.installment_ids:
                ...
                increased_installments = [
                    rec.get_future_value(rec.installment_amount,
                                         rec.increasement_rate,
                                         math.floor(i / installments_count)) for
                    i in period_range]
    ...
    def create_initial_bill(self):
        ...
        if self.leasor_type == 'single':
            self.create_single_initial_bill(self.vendor_id, amount,
            ...
            for leasor in self.multi_leasor_ids:
                ...
                self.create_single_initial_bill(partner, leasor_amount,
                                                incentives_received)

    def create_single_initial_bill(self, partner, amount, incentives_received):
    ...
    def create_commencement_move(self):
        ...
        if short_lease_liability_amount or self.initial_payment_value:
            lines.append((0, 0, {
                ...
                'debit': 0,
                'credit': short_lease_liability_amount + self.initial_payment_value,
                'analytic_account_id': self.analytic_account_id.id,
                'project_site_id': self.project_site_id.id,
                'type_id': self.type_id.id,
                'location_id': self.location_id.id,
    ...
    def create_beginning_installments(self, remaining_lease_liability):
        ...
        for i in period_range:
            ...
            if i > 0:
                amount = self.get_future_value(self.installment_amount,
    ...
    def create_interset_move(self, installment, move_date, interest_amount):
        if round(interest_amount, 3) > 0:
            lease_account_id = self.lease_liability_account_id.id if (
                ...
                'use_short') or (self._context.get(
                'reassessment') and not installment.is_long_liability)) else self.long_lease_liability_account_id.id
            lines = [(0, 0, {
    ...
    def get_installment_entry_amount(self, installment):
        ...
        if self.payment_method == 'beginning':
            ...
            if install_period >= installments_per_year:
        ...
        for i, ins in enumerate(installments_to_modify):
            ...
            if i:
                if reassessment_date:
                    ins.interest_move_ids.filtered(
                        lambda m: m.date > reassessment_date).sudo().unlink()
                else:
