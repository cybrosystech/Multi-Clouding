...
class LeaseeContract(models.Model):
    ...
    def create_initial_bill(self):
        ...
        if self.leasor_type == 'single':
            self.create_single_initial_bill(self.vendor_id, amount, self.incentives_received)
        else:
            for leasor in self.multi_leasor_ids:
                partner = leasor.partner_id
                ...
                self.create_single_initial_bill(partner, leasor_amount, incentives_received)
    ...
    def get_commencement_short_amount(self):
        ...
        if self.payment_method == 'beginning':
            ...
            installment = self.installment_ids.filtered(lambda i: i.get_period_order() == 1)
    ...
    def get_installment_entry_amount(self, installment):
        ...
        if self.payment_method == 'beginning':
            ...
            # else:
            if install_period == 1:
                installments = self.installment_ids.filtered(lambda i: i.get_period_order() <= (installments_per_year + 1) and i.get_period_order() > 1)
        ...
        for i, ins in enumerate(installments_to_modify):
            ...
            if i:
                if reassessment_date:
                    ins.interest_move_ids.filtered(lambda m: m.date > reassessment_date).sudo().unlink()
                else:
