...
class PaymentApprovalCheck(models.Model):
    _name = 'purchase.requisition.approval.check'
    _description = 'Purchase Requisition Approval Check'
    _rec_name = 'name'

    name = fields.Char(string="Name", required=False, )
    purchase_requisition_approval_line_ids = fields.One2many(
        comodel_name="purchase.requisition.approval.check.line",
        inverse_name="purchase_requisition_approval_check_id",
        string="",
        required=False, )
    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env.company)

    @api.constrains('purchase_requisition_approval_line_ids')
    def check_lines(self):
        for rec in self:
            count_map = {}
            for line in rec.purchase_requisition_approval_line_ids:
...
class PaymentApprovalCheckLine(models.Model):
    _name = 'purchase.requisition.approval.check.line'
    _description = 'Purchase Requisition Approval Check Line'
    _rec_name = 'name'

    purchase_requisition_approval_check_id = fields.Many2one(
        comodel_name="purchase.requisition.approval.check",
    ...
    company_id = fields.Many2one('res.company', 'Company',
                                 related='purchase_requisition_approval_check_id.company_id',
                                 store=True)
